...
def main(args):
    ...
    class ClientTune(nn.Module):
        def __init__(self, backbone='resnet18', classes=60):
            super(ClientTune, self).__init__()
            if backbone == 'mobilenetv3':
                self.encoder = torchvision.models.mobilenet_v3_small(pretrained=True)
                self.encoder.classifier[3] = torch.nn.Identity()
                self.final_proj = nn.Linear(1024, classes)
            elif backbone == 'resnet18':
                self.encoder = torchvision.models.resnet18(pretrained=True)
                self.encoder.fc = torch.nn.Identity()
                self.final_proj = nn.Linear(512, classes)
                ...
                self.final_proj = nn.Linear(1280, classes)
            elif backbone == 'vgg16':
                self.encoder = torchvision.models.vgg16(pretrained=True)
                self.encoder.classifier[6] = torch.nn.Identity()
                self.final_proj = nn.Linear(4096, classes)
    ...
    nicopp_path = "/home/seyda/FedLMG-try/FedLMG/datasets/NICO_DG/dim"
    ...
    if not os.path.exists(f'gen_data/nicopp/'+class_prompts[args.category]):
        os.makedirs(f'gen_data/nicopp/'+class_prompts[args.category])
    ...
    for client in range(client_num):
        ...
        model = ClientTune(backbone=backbone, classes=60)
        path = f'output/nicopp_img10_{client}_epoch_19_{backbone}.tar'
        print(path)
                ...
                for repeat in range(1):
                    ...
                    with accelerator.accumulate(unet):
                        ...
                        for aaaa in range(10):
                            ...
                            loss_bn = sum([h.r_feature for h in hooks]) if hooks else torch.tensor(0.0, device=latents.device, dtype=latents.dtype) # bn层loss
                            loss_oh = F.cross_entropy(output.unsqueeze(0), torch.tensor(step).repeat(bsz).to(unet.device))  # ce_loss
                            loss = 0.1 * loss_bn if hooks else loss_oh
                    ...
                    if xxxx%1==0:
                        with torch.no_grad():
                           ...
                           # if nsfw_content_detected[0] == False and loss_oh <= 0.5 and torch.argmax(output.unsqueeze(0)) == step:
                            ...
                            torchvision.utils.save_image(image, 'gen_data/nicopp/'+class_prompts[args.category]+'/train_prop'+str(client)+'_'+str(img_num)+'.jpg')
                            img_num = img_num + 1
                            xxxx = xxxx + 1
                            #else :
                                #xxxx = xxxx + 1
