    ...
    def train(self,client,lr,epochs,test_data,change_backbone=None):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for i, (image, label) in enumerate((self.dataloader)):
                ...
                optimizer.step()
            scheduler.step()
            print(f'epoch {epoch} | train_loss: {loss.detach().item():.4f} | lr: {scheduler.get_last_lr()[0]}')
            if epoch%5 ==0:
                top1, topk, test_loss = evaluation(self.model,test_data)
                print(f'epoch {epoch} | test_loss: {test_loss:.4f} | top1: {top1:.2f} | top5: {topk:.2f}')    
        self.model.eval()
        ...
        top1, topk, test_loss = evaluation(self.model,test_data)
        print(f'final | test_loss: {test_loss:.4f} | top1: {top1:.2f} | top5: {topk:.2f}')  
